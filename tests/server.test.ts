...
describe("CalculatorMcpServer", () => {
    let server: CalculatorMcpServer;

    // Setup once for all tests to avoid memory leaks
    function setup() {
        if (!server) {
    ...
    describe("Server Initialization", () => {
        ...
        test("should have correct server configuration", async () => {
            setup();
            // Server configuration is set during construction
            strictEqual(server instanceof CalculatorMcpServer, true);
            // We can't easily access private server config without exposing it
