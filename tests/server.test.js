...
describe("CalculatorMcpServer", () => {
    ...
    describe("Server Initialization", () => {
        test("should create server instance successfully", () => {
            setup();
            strictEqual(server instanceof CalculatorMcpServer, true);
        });
        test("should have correct server configuration", async () => {
            setup();
            // Access private server property for testing
            const serverConfig = server.server.config;
            strictEqual(serverConfig.name, "calculator-mcp-server");
            strictEqual(serverConfig.version, "1.0.0");
            strictEqual(serverConfig.description, "A simple MCP server for adding two numbers");
        });
    });
    describe("Addition Tool", () => {
        ...
        describe("Invalid Inputs", () => {
            test("should handle Infinity inputs", async () => {
                ...
                deepStrictEqual(response.operands, [Infinity, 5]);
            });
            test("should handle -Infinity inputs", async () => {
                setup();
                const mockTool = await getMockAdditionTool();
                ...
                strictEqual(response.error, "Failed to add numbers");
                strictEqual(response.details, "Both inputs must be finite numbers");
                deepStrictEqual(response.operands, [-Infinity, 10]);
            });
            test("should handle NaN inputs", async () => {
                ...
                strictEqual(response.error, "Failed to add numbers");
                strictEqual(response.details, "Both inputs must be finite numbers");
                deepStrictEqual(response.operands, [NaN, 5]);
            });
            test("should handle both inputs as NaN", async () => {
                ...
                strictEqual(response.error, "Failed to add numbers");
                strictEqual(response.details, "Both inputs must be finite numbers");
                deepStrictEqual(response.operands, [NaN, NaN]);
            });
        });
        describe("Edge Cases", () => {
